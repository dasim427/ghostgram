...
// GHOSTGRAM: Send delay — store outgoing messages as scheduled so the UI clears the input
// immediately and the server sends them after the delay (visible in Scheduled Messages meanwhile)
private func ghostgramApplySendDelay(_ messages: [(Bool, EnqueueMessage)], hasMedia: Bool) -> [(Bool, EnqueueMessage)] {
    guard SendDelayManager.shared.isEnabled else {
        return messages
    }
    let delayInterval = hasMedia ? SendDelayManager.mediaDelaySeconds : SendDelayManager.textDelaySeconds
    let scheduleTime = Int32(Date().timeIntervalSince1970) + Int32(delayInterval)
    return messages.map { transformed, message in
        let updatedMessage = message.withUpdatedAttributes { attributes in
        ...
        return (transformed, updatedMessage)
    }
}
...
public func enqueueMessages(account: Account, peerId: PeerId, messages: [EnqueueMessage]) -> Signal<[MessageId?], NoError> {
    ...
    let delayedSignal = signal
    |> map { messages -> [(Bool, EnqueueMessage)] in
        return ghostgramApplySendDelay(messages, hasMedia: hasMedia)
    }
